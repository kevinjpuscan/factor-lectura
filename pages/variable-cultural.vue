...
<script>
import QuestionsMain from "../components/pages/variable-cultural/questionsMain";
...
export default {
  ...
  computed: {
    questions() {
      return [...this.$store.state.varCultural.questionsMain];
    },
    isDisabled() {
      let disabled = false;
  ...
  methods: {
    async save() {
      ...
      let response = await repository.save("/var-cultural", body);
      this.loading = false;
